...
function Convert-TxtToHtml {
    ...
    foreach ($line in $lines) {
        ...
        #$processedText = [regex]::Replace($processedText, '(?<!<[^>]*)(?:(?:법|영)\s+)?((?:제\d+조(?:의\d+)*|제\d+항|제\d+호(?:의\d+)*)\s+(?:본문|단서|전단|후단)|제\d+조(?:의\d+)*|제\d+항|제\d+호(?:의\d+)*|(?<![가-힣])(?:가|나|다|라|마|바|사|아|자|차|카|타|파|하)목|별지\s*제\d+호(?:의\d+)*\s*서식|별표\s*\d+(?:의\d+)*|제\d+급)', '<span class="idx-article">$0</span>')
        #$processedText = [regex]::Replace($processedText, '(?<!<[^>]*)(?:(?:법|영)\s+)?((?:제\d+조(?:의\d+)*|제\d+항|제\d+호(?:의\d+)*)\s+(?:본문|단서|전단|후단)|제\d+조(?:의\d+)*|제\d+항|제\d+호(?:의\d+)*|(?<![가-힣])(?:가|나|다|라|마|바|사|아|자|차|카|타|파|하)목(?![가-힣])|별지\s*제\d+호(?:의\d+)*\s*서식|별표\s*\d+(?:의\d+)*|제\d+급)', '<span class="idx-article">$0</span>')
